/* ======================================================================
   FULL SCRIPT — SCD TYPE 2 VERSION (RUN IN DBEAVER)
   Changes from Type 1:
     - DW_DIM_PRODUCT / SHOP / CAMPAIGN: +EFFECTIVE_DATE, END_DATE, IS_CURRENT
     - Removed UNIQUE on natural keys (PRD_ID, SHOP_ID, CMP_ID)
     - ETL: MERGE replaced with UPDATE(close) + INSERT(new version)
     - Fact load: JOIN on date range for correct version
   ====================================================================== */
...
/* =========================
   2) LOOKUP DIMS (copy structure — no SCD needed)
   ========================= */
...
CREATE TABLE DW_DIM_SHOP (
  ...
  SHOP_ID        NUMBER NOT NULL,              -- natural key (can repeat)
  SHOP_NAME      VARCHAR2(200),
  RATING_AVG     NUMBER(5,2),
  USR_ID         NUMBER,
  CREATED_AT     DATE,
  UPDATED_AT     DATE,
  EFFECTIVE_DATE DATE   DEFAULT SYSDATE,       -- SCD2: version start
  END_DATE       DATE   DEFAULT DATE '9999-12-31', -- SCD2: version end
  IS_CURRENT     CHAR(1) DEFAULT 'Y' CHECK (IS_CURRENT IN ('Y','N'))
  -- NO UNIQUE on SHOP_ID (multiple versions allowed)
);

CREATE INDEX IX_DW_DIM_SHOP_CURRENT ON DW_DIM_SHOP (SHOP_ID, IS_CURRENT);
CREATE INDEX IX_DW_DIM_SHOP_DATERANGE ON DW_DIM_SHOP (SHOP_ID, EFFECTIVE_DATE, END_DATE);

-- ==================== DW_DIM_PRODUCT (SCD2) ====================
CREATE TABLE DW_DIM_PRODUCT (
  PRD_KEY        NUMBER PRIMARY KEY,
  PRD_ID         NUMBER NOT NULL,              -- natural key (can repeat)
  ...
  CREATED_AT     DATE,
  UPDATED_AT     DATE,
  EFFECTIVE_DATE DATE   DEFAULT SYSDATE,       -- SCD2
  END_DATE       DATE   DEFAULT DATE '9999-12-31',
  IS_CURRENT     CHAR(1) DEFAULT 'Y' CHECK (IS_CURRENT IN ('Y','N'))
);

CREATE INDEX IX_DW_DIM_PRODUCT_CURRENT ON DW_DIM_PRODUCT (PRD_ID, IS_CURRENT);
CREATE INDEX IX_DW_DIM_PRODUCT_DATERANGE ON DW_DIM_PRODUCT (PRD_ID, EFFECTIVE_DATE, END_DATE);

-- ==================== DW_DIM_CAMPAIGN (SCD2) ====================
CREATE TABLE DW_DIM_CAMPAIGN (
  CMP_KEY        NUMBER PRIMARY KEY,
  CMP_ID         NUMBER NOT NULL,              -- natural key (can repeat)
  ...
  UPDATED_AT     DATE,
  EFFECTIVE_DATE DATE   DEFAULT SYSDATE,       -- SCD2
  SCD_END_DATE   DATE   DEFAULT DATE '9999-12-31', -- renamed to avoid conflict with campaign END_DATE
  IS_CURRENT     CHAR(1) DEFAULT 'Y' CHECK (IS_CURRENT IN ('Y','N'))
);
...
/* =========================
   6) FACT TABLES (unchanged structure)
   ========================= */
...
CREATE OR REPLACE PROCEDURE PR_ETL_FULL_REFRESH AS
  ...
  v_initial_date DATE := DATE '2000-01-01';  -- SCD2: first version covers all past dates
BEGIN
  /* ========== LOAD STAGING ========== */
  ...
  /* ========== LOOKUP DIMS (full refresh, no SCD) ========== */
  ...
  /* ==========================================================
     CORE DIM: SHOP — SCD TYPE 2
     Step 1: Close old rows where data changed
     Step 2: Insert new version rows + brand new shops
     ========================================================== */

  -- Step 1: Close changed rows
  UPDATE DW_DIM_SHOP d
  SET d.END_DATE   = v_today - 1,
      d.IS_CURRENT = 'N'
  WHERE d.IS_CURRENT = 'Y'
    AND EXISTS (
      SELECT 1 FROM STG_SHOP s
      WHERE s.SHOP_ID = d.SHOP_ID
        AND (   UPPER(TRIM(s.SHOP_NAME)) != d.SHOP_NAME
             OR ROUND(NVL(s.RATING_AVG,0),2) != NVL(d.RATING_AVG,0)
             OR NVL(s.USR_ID,0)              != NVL(d.USR_ID,0)
        )
    );

  -- Step 2: Insert new versions (changed + new)
  ...
  SELECT
    ...
    s.CREATED_AT,
    s.UPDATED_AT,
    CASE WHEN EXISTS (SELECT 1 FROM DW_DIM_SHOP x WHERE x.SHOP_ID = s.SHOP_ID)
         THEN v_today          -- changed record: new version starts today
         ELSE v_initial_date   -- brand new: cover all historical orders
    END,
    DATE '9999-12-31',
    'Y'
  FROM STG_SHOP s
  ...
  WHERE d.IS_CURRENT = 'Y'
    AND EXISTS (
      ...
      WHERE s.PRD_ID = d.PRD_ID
        AND (   UPPER(TRIM(s.NAME)) != d.PRD_NAME
             OR ROUND(CASE WHEN NVL(s.PRICE,0)<0 THEN 0 ELSE NVL(s.PRICE,0) END, 2) != NVL(d.PRICE,0)
             OR (CASE WHEN NVL(s.STOCK,0)<0 THEN 0 ELSE NVL(s.STOCK,0) END)          != NVL(d.STOCK,0)
             OR ROUND(CASE WHEN NVL(s.DISCOUNT,0)<0 THEN 0 ELSE NVL(s.DISCOUNT,0) END, 2) != NVL(d.DISCOUNT,0)
  ...
  SELECT
    ...
    ROUND(CASE WHEN NVL(s.PRICE,0)<0    THEN 0 ELSE NVL(s.PRICE,0)    END, 2),
    CASE  WHEN NVL(s.STOCK,0)<0         THEN 0 ELSE NVL(s.STOCK,0)    END,
    ROUND(CASE WHEN NVL(s.DISCOUNT,0)<0 THEN 0 ELSE NVL(s.DISCOUNT,0) END, 2),
    s.CAT_ID,
    s.PRD_TYPE_ID,
    s.SHOP_ID,
    s.CREATED_AT,
    s.UPDATED_AT,
    CASE WHEN EXISTS (SELECT 1 FROM DW_DIM_PRODUCT x WHERE x.PRD_ID = s.PRD_ID)
         THEN v_today
         ELSE v_initial_date
    END,
    DATE '9999-12-31',
  ...
  WHERE NOT EXISTS (
    ...
    WHERE d.PRD_ID = s.PRD_ID
      AND d.IS_CURRENT = 'Y'
  );

  /* ==========================================================
     CORE DIM: CAMPAIGN — SCD TYPE 2
     ========================================================== */
  ...
  WHERE d.IS_CURRENT = 'Y'
    AND EXISTS (
      ...
      WHERE s.CMP_ID = d.CMP_ID
        AND (   UPPER(TRIM(s.NAME)) != d.CMP_NAME
             OR ROUND(CASE WHEN NVL(s.DISCOUNT,0)<0 THEN 0 ELSE NVL(s.DISCOUNT,0) END, 2) != NVL(d.DISCOUNT,0)
             OR NVL(s.START_DATE, DATE '1900-01-01') != NVL(d.START_DATE, DATE '1900-01-01')
             OR NVL(s.END_DATE,   DATE '1900-01-01') != NVL(d.END_DATE,   DATE '1900-01-01')
  ...
  INSERT INTO DW_DIM_CAMPAIGN (
    ...
    CREATED_AT, UPDATED_AT,
    EFFECTIVE_DATE, SCD_END_DATE, IS_CURRENT
  )
  SELECT
    ...
    s.CREATED_AT,
    s.UPDATED_AT,
    CASE WHEN EXISTS (SELECT 1 FROM DW_DIM_CAMPAIGN x WHERE x.CMP_ID = s.CMP_ID)
         THEN v_today
         ELSE v_initial_date
    END,
    DATE '9999-12-31',
  ...
  SELECT
    ...
    USR_ID,
    PAY_STAT_ID,
    ROUND(CASE WHEN NVL(TOTAL_AMOUNT,0)   < 0 THEN 0 ELSE NVL(TOTAL_AMOUNT,0)   END, 2),
    ROUND(CASE WHEN NVL(TOTAL_DISCOUNT,0) < 0 THEN 0 ELSE NVL(TOTAL_DISCOUNT,0) END, 2)
  FROM STG_ORDERS;

  /* ==========================================================
     FACT: ORDER LINE — SCD TYPE 2 JOIN
     Key change: JOIN dims on date range instead of simple NK match
     ========================================================== */
  ...
  /* ---- SCD2 JOIN: match product version active at order date ---- */
  JOIN DW_DIM_PRODUCT pdim
    ON  pdim.PRD_ID = p.PRD_ID
    AND TRUNC(o.ORDER_DATE) >= pdim.EFFECTIVE_DATE
    AND TRUNC(o.ORDER_DATE) <= pdim.END_DATE
  /* ---- SCD2 JOIN: match shop version active at order date ---- */
  JOIN DW_DIM_SHOP sdim
    ON  sdim.SHOP_ID = p.SHOP_ID
    AND TRUNC(o.ORDER_DATE) >= sdim.EFFECTIVE_DATE
    AND TRUNC(o.ORDER_DATE) <= sdim.END_DATE;
