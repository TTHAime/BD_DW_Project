-- =========================================================
-- OPTIONAL: DROP เฉพาะ objects ชุดนี้ก่อน 
-- =========================================================
BEGIN
  FOR x IN (SELECT trigger_name FROM user_triggers
            WHERE trigger_name IN ('TRG_ORD_DTL_SEQ','TRG_CMP_PRD_SEQ')) LOOP
    EXECUTE IMMEDIATE 'DROP TRIGGER ' || x.trigger_name;
  END LOOP;

  FOR x IN (SELECT table_name FROM user_tables
            WHERE table_name IN (
              'CMP_PRD','CAMPAIGN','ORD_DTL','ORDERS','PRODUCT','SHOP',
              'PERIOD','SHP_STAT','PAY_STAT','PRD_TYPE','CATEGORY','APP_USER','USR_TYPE'
            )) LOOP
...
-- ✅ FIXED: เพิ่ม CAT_ID + ใส่ comma ก่อน CONSTRAINT
